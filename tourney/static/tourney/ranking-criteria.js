...
class RankingCriteriaManager {
  constructor(containerSelector, inputSelector = null) {
    this.availableCriteria = [
      { key: 'wins', name: 'Wins', description: 'Number of match wins' },
      {
        key: 'losses',
        name: 'Losses',
        description: 'Number of match losses (fewer is better)',
      },
      {
        key: 'points',
        name: 'Points',
        description: 'Number of points (2 for win, 1 for tie)',
      },
      {
        key: 'strength_of_schedule',
        name: 'Strength of Schedule',
        description: 'Average points of opponents faced',
      },
      {
        key: 'head_to_head',
        name: 'Head-to-Head',
        description: 'Wins against tied opponents',
      },
      {
        key: 'seed',
        name: 'Seed',
        description: 'Original tournament seed (lower is better)',
      },
      { key: 'random', name: 'Random', description: 'Random tiebreaker' },
      {
        key: 'player_score',
        name: 'Player Score',
        description: 'Sum of player scores in current stage',
      },
      {
        key: 'opponent_score',
        name: 'Opponent Score',
        description: 'Sum of opponent scores (lower is better)',
      },
      {
    ...
    this.criteriaInput = inputSelector
  ...
  renderCriteria() {
    ...
    this.currentCriteria.forEach((criterion, index) => {
      const criteriaData = this.availableCriteria.find(
        (c) => c.key === criterion.key
      )
      if (!criteriaData) return

      const item = document.createElement('div')
      item.className = `drag-drop-item ranking-criteria-item ${
  ...
  setupToggleSwitches() {
    ...
    toggles.forEach((toggle) => {
      toggle.addEventListener('click', () => {
        ...
        this.currentCriteria[index].enabled =
          !this.currentCriteria[index].enabled
        this.renderCriteria()
      })
    })
  ...
  setupDragAndDrop() {
    const items = this.criteriaContainer.querySelectorAll(
    ...
    const dropIndicators =
      this.criteriaContainer.querySelectorAll('.drop-indicator')
